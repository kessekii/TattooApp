...
const ProfilePageComponent: React.FC<ProfilePageProps> = ({ profileData, setProfileData }) => {
    ...
    const [isFollowing, setIsFollowing] = useState(profileData.friends.filter((friend) => friend.username === username).length > 0);
    const [friendFollowing, setFriendFollowing] = useState(new Array(profileData.friends.length).fill(false));
    ...
    const handleReviewSubmit = () => {
        const updatedReviews = [...profileData.reviews, { photo: 'new-photo.jpg', nickname: username, ...newReview }];
    ...
    const handleFriendFollowClick = (index: number, friend: any, follow: boolean) => {
        ...
        console.log('New Post:', friend, profileData.friends, username, friend.username);
        const updatedProfileData = { ...profileData };
        const filteredFriends = profileData.friends.filter((frie) => frie.username !== friend.username);
        if (filteredFriends == profileData.friends) {
            updatedProfileData.friends = [...profileData.friends, newPost]
        } else {
            updatedProfileData.friends = filteredFriends;
        }
        console.log('Filtered Friends:', filteredFriends);
        updatedProfileData.friends = follow ? [...profileData.friends, newPost] : filteredFriends;
    ...
    const setClientDate = (date: any) => {
        ...
        const filteredDates = profileData.calendar.filter((d, i) => {
            if (d.date === date.date) {
                index = i;
                return true;
            }
        ...
        if (index !== -1) {
            ...
            const existingHours = profileData.calendar[index].hours;
            ...
            if (hourIndex !== -1) {
                ...
                profileData.calendar[index].hours = existingHours.filter((hour) => hour !== date.time);
                console.log(`Time ${date.time} removed from the existing date`, profileData.calendar[index].hours);
            } else {
                // Add the new hour if it doesn't exist
                profileData.calendar[index].hours.push(date.time);
                console.log(`Time ${date.time} added to the existing date`, profileData.calendar[index].hours);
            }

            // Update the state
            const updatedProfileData = { ...profileData };
            updatedProfileData.calendar[index].hours = [...profileData.calendar[index].hours];
            setProfileData(updatedProfileData);
        } else {
    ...
    const setMasterDate = (calendar: { dates, hours }) => {
        ...
        let finalDates = calendar.dates.map((d) => ({ date: d, hours: calendar.hours }));
        ...
        profileData.calendar = finalDates;
        setProfileData(profileData);
        console.log('Updated Calendar:', profileData);
        return profileData.calendar;
    }
    ...
    const handleFollow = () => {
        ...
        const updatedFriends = isFollowing
            ? profileData.friends.filter(friend => friend.username !== username)
            : [...profileData.friends, { avatar: profileData.profilePicture, nickname: profileData.name, username: profileData.username }];
        setProfileData(prevProfile => ({
            ...prevProfile,
        ...
        <ProfilePage>
                    ...
                    {isEditing ? (
                        <InfoGrid>
                            ...
                            <EditInput
                                ...
                                onChange={handleInputChange}
                            />
                            <EditButton onClick={handleLocationEditClick}>Edit Location</EditButton>
                        </InfoGrid>
                    ) : (
                        <InfoGrid>
                            <ProfilePicture src={profileData.profilePicture} alt="Profile Picture" />
                            <div >

                                <h1>{profileData.name}</h1>

                                <p style={{ color: "black" }}>{profileData.description}</p>
                                <p style={{ color: "black" }}>Location: {profileData.location}</p>
                            </div>
                        </InfoGrid>
                    )}
                        ...
                        {loggedInUser ?
                            isEditing ? (
                                ...
                                <>
                                    ...
                                    <CalendarButton onClick={handleCalendarClick}>Open Calendar</CalendarButton>
                                    <LinksButton onClick={handleLinksClick}>Social Media Links</LinksButton>
                                    <ReviewButton onClick={() => setShowReviews(true)}>
                                        Show Reviews ({profileData.reviews.length})
                                    </ReviewButton>
                                </>
                            ) : (
                                ...
                                <>
                                    ...
                                    <FriendsSection>
                                        <h2>Friends</h2>
                                        <FriendsAvatars>
                                            {profileData.friends.slice(0, 3).map((friend, index) => (
                                                <FriendAvatar
                                                    key={index}
                                                    src={friend.avatar}
                                                    alt={friend.nickname}
            ...
            <ProfilePosts>
                {profileData.posts.map((post) => (
                    <Post key={post.id}>
                        <PostImage src={post.image} onClick={() => { navigate("../" + username + "/portfolio") }} alt={`Post ${post.id}`} />
                    </Post>
                ))}
            </ProfilePosts>
                ...
                showReviews && (
                    <ReviewPopup>
                        <ReviewContent>
                            ...
                            {!loggedInUser && <AddReviewButton onClick={handleAddReviewClick}>+ Add New Review</AddReviewButton>
                            }
                            ...
                            {showAddReview && (
                                <NewReviewForm>
                                    ...
                                    />
                                    <ReviewSelect name="mark" value={newReview.mark} onChange={handleNewReviewChange}>
                                    ...
                                    <ReviewSubmitButton onClick={handleReviewSubmit}>Submit Review</ReviewSubmitButton>
                                </NewReviewForm>
                            )}
                        </ReviewContent>
                    </ReviewPopup>
                                ...
                                {profileData.friends.map((friend, index) => (
                                    ...
                                    >
                                        ...
                                        <FriendFollowButton
                                            ...
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                handleFriendFollowClick(index, friend, friendFollowing[index] ? true : false);
                                            }}
                                        >
                ...
                showCalendarPopup && (
                    ...
                    <CalendarPopup>
                        <CalendarContent>
                            ...
                            {!loggedInUser ? <>
                                <PortfolioImagePicker profileData={profile} onImageSelect={handleImageSelect} /> <CalendarComponent setClosePopup={handleCalendarClick} availableHours={profileData.calendar} setClientDate={setClientDate} />  </>
                                :
                                <DateHourSelector setScheduleUpdated={setCalendarUpdated} setMasterDate={setMasterDate} />
                            }
                            ...
                            <div>
                                ...
                                <CancelButton onClick={() => setCalendarPopup(false)}>{calendarUpdated ? 'Back' : 'Cancel'}</CancelButton>
                            </div>
                        </CalendarContent>
                    ...
                    <LocationPopup>
                        <LocationContent>
                            ...
                            <div>
                                ...
                                <CancelButton onClick={handleLocationCancelClick}>Cancel</CancelButton>
                            </div>
                        </LocationContent>
                    </LocationPopup>
                )
