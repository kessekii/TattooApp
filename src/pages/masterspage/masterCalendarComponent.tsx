...
import { TextField, Button, Grid, Box, Typography, Checkbox, FormControlLabel, FormGroup } from '@mui/material';
...
const DateHourSelector: React.FC<MasterCalendarProps> = ({ setScheduleUpdated, setMasterDate }) => {
    const [selectedStartDate, setSelectedStartDate] = useState<Date | null>(null);
    const [selectedEndDate, setSelectedEndDate] = useState<Date | null>(null);
    const [selectedHours, setSelectedHours] = useState<number[]>([]);
    const [hourRange, setHourRange] = useState<number[]>([0, 23]); // Min and Max hour
    const [hourOptions, setHourOptions] = useState<Array<{ value: number; label: string }>>([]);
    ...
    const handleSave = () => {
        if (selectedStartDate && selectedEndDate) {
            ...
            setMasterDate({ dates: dateRange.map((d) => d.toISOString().split('T')[0]), hours: selectedHours.map((hour: number) => hour.toString()) });
            console.log("Selected Dates:", dateRange.map((d) => format(d, 'yyyy-MM-dd')));
        }
    ...
    return (
        <Box display="flex" flexDirection="column" alignItems="center">
            ...
            {/* Buttons */}
            <SelectAllButton onClick={handleSelectAllDay}>Select All Day</SelectAllButton>
            <ClearAllButton onClick={handleClearAll}>Clear All</ClearAllButton>

            {selectedStartDate && selectedEndDate && (
                <SaveButton onClick={handleSave}>{selectedHours.length > 0 ? "Save Available Hours" : "Save As Inavailble"}</SaveButton>
            )}
        </Box>
    );
};
