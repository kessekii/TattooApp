...
const lerp = (value, minWidth, maxWidth, minValue, maxValue) => {
  ...
  return minValue + (maxValue - minValue) * ((clampedValue - minWidth) / (maxWidth - minWidth));
};
...
width: 100vw;
  height: 100vh;
  background-image: ${({ background }: any) => background}; /* Set your background image */
...
const Backdrop = (props: { screen }) => {
  ...
  if (screen) {
    ...
    return (
      ...
      <ClippedSvgBackground background="url('https://picsum.photos/seed/picsum/200/300')" scale={scale} />
    )
  } else {
    return <></>
  }
